...
void f(string &s1, string &s2, vector<vector<int>>& dp, int number_of_threads, int thread_index, int n) {
    for (int diagonal = 2; diagonal <= n; diagonal++) {
        ...
        int start = thread_index+1;
        for (int i = start; i <= size_of_diagonal; i += number_of_threads) {
            int j = diagonal - i;
            if (s1[i-1] == s2[j-1])
    ...
    int diagonal = n+1;
    int row = diagonal-n;
    for (int i = thread_index; i < n; i += number_of_threads) {
    ...
    for (int diagonal = n+2; diagonal <= 2*n+1; diagonal++) {
        ...
        int row = diagonal-n;
        for (int i = thread_index; i < size_of_diagonal; i += number_of_threads) {
            int j = diagonal - i - row;
            if (s1[i+row-1] == s2[j-1])
                dp[diagonal][i] = dp[diagonal-2][i+1]+1;
            else
