...
void fill_Hirschberg(string & result, string &s1, string &s2,int beg1, int beg2, int n, int m, int depth = 0) {
    ...
    int half = (n+1)/2;
    vector<vector<int>> all = vector<vector<int>>(2, vector<int>(m+2,0));
    if (depth < MAX_DEPTH) {
        thread t1([&](){fill_vector_normal(all[0], s1, s2, beg1, beg2, half, m);});
    ...
    if (depth < MAX_DEPTH) {
        thread t2([&](){fill_Hirschberg(o1, s1, s2, beg1, beg2, half, k, depth+1);});
...
void solve_set(int n){
    ...
    string ans;
    fill_Hirschberg(ans, S1, S2, 0, 0, n, n);
    cout<<ans.length()<<"\n"<<ans<<"\n";
}
